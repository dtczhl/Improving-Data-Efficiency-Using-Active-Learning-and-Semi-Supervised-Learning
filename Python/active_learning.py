...
def run_cv(train_set, target, num_class, n_sample):
    ...
    for fold_, (train_idx, val_idx) in enumerate(folds.split(train_set.values, target)):
        ...
        booster = LightGBMTuner(params=param, train_set=train_data, valid_sets=[val_data], verbose_eval=False,
                                early_stopping_rounds=100, show_progress_bar=False, keep_training_booster=True)
        booster.run()
